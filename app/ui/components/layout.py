...
NAV_LINKS = (
    ...
    ("nav.search", "Search", "/friends/search"),
    ("nav.messages", "Messages", "/messages"),
    ("nav.notifications", "Notifications", "/notifications"),
    ("nav.settings", "Settings", "/settings"),
    ("nav.media", "Media", "/media"),
    ("nav.moderation", "Moderation", "/moderation"),
)

NAV_ROLE_REQUIREMENTS = {
    "nav.moderation": {"owner", "admin"},
}

LOGO_IMAGE = f"/assets/img/social-sphere-logo.png?v={STATIC_VERSION}"


def navbar(*, active: str | None = None, t=None) -> Markup:
    translate = t or (lambda key, default=None: default or key)

    desktop_links_html: list[str] = []
    mobile_links_html: list[str] = []
    for key, default_label, href in NAV_LINKS:
        label = translate(key, default_label)
        ...
        extra_classes = " relative" if key == "nav.notifications" else ""
        indicator_desktop = (
        ...
        role_meta = NAV_ROLE_REQUIREMENTS.get(key)
        requires_attr = ""
        hidden_class = ""
        if role_meta:
            ...
            <div class=\"mx-auto flex max-w-7xl flex-wrap items-center gap-3 px-4 py-4 sm:px-6\">
                ...
                <div class=\"flex w-full flex-wrap items-center justify-between gap-2 sm:flex-nowrap sm:justify-end md:w-auto\">
                    <button id=\"theme-toggle\" class=\"flex-1 rounded-full border border-slate-700/70 px-3 py-2 text-xs font-semibold text-slate-200 transition hover:border-indigo-500 hover:text-indigo-300 sm:flex-none sm:text-sm\">
                        <span class=\"sm:hidden\">{translate("nav.theme.short", "Theme")}</span>
                        <span class=\"hidden sm:inline\">{translate("nav.theme", "Toggle Theme")}</span>
                    </button>
                    <button
                        id=\"nav-social-ai\"
                        type=\"button\"
                    ...
                    >
                        {translate("nav.socialAi", "Social AI")}
                    </button>
                    <a id=\"nav-auth-btn\" href=\"/login\" class=\"flex-1 rounded-full border border-indigo-500/40 px-4 py-2 text-center text-xs font-semibold text-indigo-300 transition hover:bg-indigo-600/20 sm:flex-none sm:text-sm\">{translate("nav.login", "Login")}</a>
                    <button id=\"mobile-nav-toggle\" type=\"button\" class=\"inline-flex items-center gap-2 rounded-full border border-slate-700/70 px-3 py-2 text-sm font-semibold text-slate-200 transition hover:border-indigo-500 hover:text-white md:hidden\" aria-controls=\"mobile-nav-panel\" aria-expanded=\"false\">
                        <span data-mobile-nav-label>{translate("nav.menu", "Menu")}</span>
...
def shell(content: str, *, active: str | None = None, t=None) -> Markup:
    nav = navbar(active=active, t=t)
    return Markup(
        f"""
        {nav}
