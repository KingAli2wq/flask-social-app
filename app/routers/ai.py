...
async def chat_with_local_model(payload: ChatRequest) -> ChatResponse:
    ...
    if not safety.allowed:
        logger.warning(
            "Blocked AI prompt due to safety violations: %s", ", ".join(v.value for v in safety.violations)
        )
        raise HTTPException(
...
__all__ = ["router", "chat_with_local_model", "ChatRequest", "ChatResponse", "build_system_prompt"]
